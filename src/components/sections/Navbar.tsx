...
import { NAV_LINKS, APP_NAV_LINKS } from "@/lib/constants";
...
export default function Navbar() {
  ...
  const activeNavLinks = session ? APP_NAV_LINKS : NAV_LINKS;
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-150 ease-in-out ${transparent ? "bg-transparent" : "bg-white shadow-sm"}`}>
      <div className="mx-auto max-w-6xl px-6">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Mobile hamburger */}
          <button type="button" onClick={() => setMenuOpen((prev) => !prev)} className="lg:hidden flex flex-col justify-center items-center w-8 h-8 gap-1.5" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen}>
